...
# RESOURCES:
from iexfinance.stocks import Stock
a = Stock("APPL", output_format='pandas', token="sk_261e4411a4ef43fab3fea00a67631841")
# iex finance stocks:
...
IEX_TOKEN = "sk_261e4411a4ef43fab3fea00a67631841"
...
def prediction_generator(investor, prediction_file = 'data/prediction.csv', horizon = 5):
    ...
    df = get_historical_data(ticker_list, start, end, output_format='pandas', token="sk_261e4411a4ef43fab3fea00a67631841")
    idx = pd.IndexSlice
    df = df.loc[:,idx[:,"close"]]
    df.columns = ticker_list
...
def button(bot, update):
    ...
    for k in range(len(ticker_list)):
        ...
        if str(query.data) == ticker_list[k]:
            ...
            # creating dictionary of data
            df = get_historical_data(str(query.data), start, end, output_format='pandas', token="sk_261e4411a4ef43fab3fea00a67631841")
            df = df.loc[:,["close"]]
...
def main():
    ...
    Run the bot
    """
    updater = Updater("586920477:AAGEG_RwFHV-J_imeNCwGEP8yhqcC7Xabv4")
